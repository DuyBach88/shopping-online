<%-- 
    Document   : Header
    Created on : 13 Oct, 2023, 11:31:22 AM
    Author     : HP
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@page import="DAL.*" %>
    <%@page import="entity.*" %>
    <%@page import="java.util.*" %>
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Document</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@10/swiper-bundle.min.css" />
        <!--<link rel="stylesheet" href="../style.css">-->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" integrity="sha512-z3gLpd7yknf1YoNbCzqRKc4qyor8gaKU1qmn+CShxbuBusANI9QpRohGBreCFkKxLhei6S9CQXFEbbKuqLg0DA==" crossorigin="anonymous" referrerpolicy="no-referrer"
              />
        <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    </head>

    <body>
        <%
            Account account = null;
            if(session.getAttribute("account") != null) {
                account = (Account)session.getAttribute("account");
            }
        %>
        <section id="header">
            <div class="container">
                <div class="row align-items-center py-3">
                    <div class="col-md-1">
                        <div class="header-logo">
                            <a href="ProductURL"><img src="data:image/svg+xml;base64," alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-5">
                        <nav class="navbar navbar-expand-lg bg-body-tertiary bg-danger">
                            <div class="container-fluid">
                                <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                                    <span class="navbar-toggler-icon"></span>
                                </button>
                            </div>
                        </nav>
                    </div>
                    <div class="col-md-4">
                        <div class="input-box d-flex align-items-center">
                            <div class="search-icon bg-black" onclick="handleSearch()">
                                <div class="border-0 d-flex">
                                    <i class='bx bx-search text-white'></i>
                                </div>
                            </div>
                            <div class="search-input" >
                                <input type="text" id="header-search"
                                       placeholder="Search" 
                                       aria-label="Example text with button addon"
                                       />
                            </div>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="row">
                            <div class="col">
                                <div class="user-status">
                                    <div class="user-logo">
                                        <i class='bx bxs-user-circle' ></i>
                                    </div>
                                    <span class="user-title">Profile</span>
                                    <div class="user-box_access" style="border-radius: 0">
                                        <div class="user-box-header">
                                            <!-- access -->
                                            <%if(account != null) {%>
                                            <div class="user-access">
                                                <div>
                                                     <%if(account.getAccount_image() != null) {%>
                                                <a href="profile" class="d-block">
                                                    <img src="./images/<%=account.getAccount_image()%>" alt=""  style="border-radius: 0">
                                                </a>
                                                <%} else {%>
                                                <a href="profile" class="d-block">
                                                    <img src="./images/accountImg.png" alt="" style="border-radius: 0">
                                                </a>
                                                <%}%>
                                                <div class="user-access-info">
                                                    <span><%=account.getFirst_name() +" "+ account.getLast_name()%></span>
                                                </div>
                                                <a href="logout" class="fs-5 text-info text-decoration-none">
                                                    <div class="logout-button">
                                                        Logout
                                                    </div>
                                                </a>
                                                </div>
                                                <%if(account.getIs_admin()) {%>
                                                <div class="bg-danger rounded-lg py-2 px-3">
                                                  <a href="ManageProduct" class="fs-5 text-white 
                                                      fw-bold text-decoration-none">
                                                    <div class="logout-button">
                                                        Manage page
                                                    </div>
                                                  </a>
                                                </div>
                                                <%}%>
                                            </div>
                                            <%} else {%>
                                            <!-- not access -->
                                            <div class="user-not-access">
                                                <a href="login" class="text-decoration-none fs-3 w-25">
                                                    <div class="bg-danger text-white px-3 py-2 rounded-lg text-center">
                                                        Login
                                                    </div>
                                                </a>
                                            </div>
                                            <%}%>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col">
                                <a href="CartURL" class="cart-status text-black text-decoration-none">
                                    <div class="cart-logo">
                                        <i class='bx bxs-cart-alt' ></i>
                                    </div>
                                    <span class="cart-title">Bag ${sessionScope.numberOrder}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </body>
</html>
